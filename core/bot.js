...
async loadCookiesFromJson(path = './cookies.json') {
  ...
  // NEW: Add proper media sending methods for Instagram
  ...
  async sendVoice(threadId, voicePath, duration = 0) {
    if (!threadId || !voicePath) {
      this.log('WARN', '⚠️ sendVoice called with missing threadId or voicePath');
    ...
    try {
      ...
      // For voice messages, we need to send as audio
      await thread.broadcastVoice({
        file: await fs.readFile(voicePath),
      ...
      try {
        ...
        await thread.broadcastPhoto({
          file: await fs.readFile(voicePath),
          text: '🎤 Voice message (sent as file)'
        });
        this.log('INFO', `🎤 Voice message sent as photo fallback to thread ${threadId}`);
        return true;
      } catch (fallbackError) {
        this.log('ERROR', `❌ Voice fallback also failed:`, fallbackError.message);
        throw error;
      }
    }
  }
  ...
  // Generic media sending method that chooses the right approach
  async sendMedia(threadId, mediaPath, caption = '') {
    if (!threadId || !mediaPath) {
      this.log('WARN', '⚠️ sendMedia called with missing threadId or mediaPath');
      throw new Error('Thread ID and media path are required');
    }
    ...
    try {
      ...
      switch (mediaType) {
        case 'image':
          return await this.sendPhoto(threadId, mediaPath, caption);
        case 'video':
          return await this.sendVideo(threadId, mediaPath, caption);
        case 'audio':
          return await this.sendVoice(threadId, mediaPath);
        default:
          // For unknown types, try as photo first, then fallback to text
          try {
            return await this.sendPhoto(threadId, mediaPath, caption);
          } catch (photoError) {
            this.log('WARN', `⚠️ Couldn't send as photo, sending filename as text: ${mediaPath}`);
            return await this.sendMessage(threadId, `📎 File: ${mediaPath.split('/').pop()}${caption ? `\n${caption}` : ''}`);
          }
      }
    } catch (error) {
      this.log('ERROR', `❌ Error sending media to thread ${threadId}:`, error.message);
      throw error;
    }
  }
